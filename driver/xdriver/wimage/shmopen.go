...
func ShmOpen(size int) (shmid, addr uintptr, err error) {
	shmid, _, errno0 := syscall.Syscall(syscall.SYS_SHMGET, ipcPrivate, uintptr(size), 0600)
	if errno0 != 0 {
		return 0, 0, fmt.Errorf("shmget: %v", errno0)
	}
	p, _, errno1 := syscall.Syscall(syscall.SYS_SHMAT, shmid, 0, 0)
	if errno1 != 0 {
...
func ShmClose(shmid, addr uintptr) error {
	...
	_, _, errno2 := syscall.Syscall(syscall.SYS_SHMCTL, shmid, ipcRmID, 0)
	if errno != 0 {
