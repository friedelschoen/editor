...
func (d *Drawer) RangeVisibleOffset(offset, length int, align drawer.RangeAlignment) int {
	...
	switch align {
	case drawer.RAlignKeep:
		return d.alignKeep()
	case drawer.RAlignKeepOrBottom:
		if v, ok := d.rangeVisibleOffsetKeepIfVisible(offset, length); ok {
			return v
		}
		return topLines(freeLines())
	case drawer.RAlignTop:
		return topLines(0)
	case drawer.RAlignCenter:
		return topLines(freeLines() / 2)
	case drawer.RAlignBottom:
		return topLines(freeLines())
	case drawer.RAlignAuto:
...
func (d *Drawer) rangeVisibleOffsetAuto(offset, length int) int {
	align := func(a drawer.RangeAlignment) int {
	...
	if v1 == VisibilityFull {
		...
		if v2 == VisibilityFull {
			...
			return align(drawer.RAlignCenter)
		}
	}
