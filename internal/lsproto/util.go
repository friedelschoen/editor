...
func RangeToOffsetLen(rd ioutil.ReaderAt, rang *Range) (int, int, error) {
	...
	lso1, err := parser.LineColumnIndex(rd, l1, 1)
	if err != nil {
		return 0, 0, err
	}
	lso2, err := parser.LineColumnIndex(rd, l2, 1)
	if err != nil {
		return 0, 0, err
	}
...
func UrlToAbsFilename(url string) (string, error) {
	return parser.UrlToAbsFilename(url)
}

func AbsFilenameToUrl(filename string) (string, error) {
	return parser.AbsFilenameToUrl(filename)
}
...
func LocationsToString(locations []*Location, baseDir string) (string, error) {
	...
	for _, loc := range locations {
		...
		res = append(res, &loc2{filename, line, col})
	}

	sort.Slice(res, func(a, b int) bool {
		if res[a].a == res[b].a {
			if res[a].b == res[b].b {
				return res[a].c < res[b].c
			}
			return res[a].b < res[b].b
		}
