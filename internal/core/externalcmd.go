...
func cmdPartArgs(part *toolbarparser.Part) []string {
	...
	for _, a := range part.Args {
		s := a.String()
		if !parser.IsQuoted(s) {
			s = parser.RemoveEscapesEscapable(s, command.EscapeCharacter(), "|")
		}
		u = append(u, s)
	}
	return u
...
func cmdVar_edFileLine(erow *ERow) string {
	...
	l, _, err := parser.IndexLineColumn(ta.RW(), ta.CursorIndex())
	if err != nil {
		return ""
	}
