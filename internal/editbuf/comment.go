...
func Comment(ctx *EditorBuffer) error {
	...
	for i := a; i < b; {
		...
		j, _, err := ioutil.RuneIndexFn(rd, i, false, isSpaceExceptNewline)
		if err != nil && !errors.Is(err, io.EOF) {
			return err
		}

		rd2 := ctx.LocalReader(j)
		u, uIsNewline, err := ioutil.LineEndIndex(rd2, j)
		if err != nil {
			return err
		}
	...
	for i := a; i < b; {
		...
		u, _, err := ioutil.LineEndIndex(rd2, i)
		if err != nil {
			return err
		}
		...
		if !empty {
			...
			u2, size, err := ioutil.RuneLastIndexFn(rd2, u, false, unicode.IsSpace)
			if err != nil {
				return err
			}
			u2 += size
...
func Uncomment(ctx *EditorBuffer) error {
	sym := ctx.Fns.CommentLineSym()
	if sym == nil {
		return nil
	}
	...
	for i := a; i < b; {
		...
		switch t := sym.(type) {
		case string:
			if ioutil.HasPrefix(ctx.RW, i, []byte(t)) {
				lines++
		...
		case [2]string:
			if ioutil.HasPrefix(ctx.RW, i, []byte(t[0])) &&
				ioutil.HasSuffix(ctx.RW, u2, []byte(t[1])) {
				lines++
