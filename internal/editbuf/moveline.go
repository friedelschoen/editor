...
func MoveLineUp(ctx *EditorBuffer) error {
	...
	a2, err := ioutil.LineStartIndex(rd, a-1) // start of previous line, -1 is size of '\n'
	if err != nil {
		return err
	}
	...
	if ctx.C.HaveSelection() {
		b2 := a2 + len(s)
		_, size, err := ioutil.ReadLastRuneAt(ctx.RW, b2)
		if err != nil {
			return nil
		}
...
func MoveLineDown(ctx *EditorBuffer) error {
	a, b, newline, err := ctx.CursorSelectionLinesIndexes()
	if err != nil {
		return err
	}
	...
	a2, newline, err := ioutil.LineEndIndex(rd2, a)
	if err != nil {
		return err
	}
